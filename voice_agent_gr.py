...
def text_to_speech(response_text):
    ...
    tts.save('gTTS.mp3')
    os.system('cmdmp3 gTTS.mp3')  # Windows-specific 播放音訊的指令
  
model = genai.GenerativeModel(model_gem)

def generate_text(audio, text):
    # audio 為 (sampling_rate, data) 格式
    sr, data = audio
    if data.ndim > 1:  # 若是多通道音訊，取最小值作為單聲道
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column():
            # text_input = gr.Text(label="Input Phrase")
            audio_input = gr.Audio(sources=["microphone"])
            btn = gr.Button("Clear")
            btn = gr.Button("Submit")

        with gr.Column():
            user_input = gr.Text(label="User Speak")
            response = gr.Text(label="Generated Response")
    btn.click(generate_text, inputs=[audio_input], outputs=[user_input, response])
